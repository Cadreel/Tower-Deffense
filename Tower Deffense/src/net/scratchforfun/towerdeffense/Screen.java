...
public class Screen extends JPanel implements Runnable {
	...
	public void paintComponent(Graphics g) {
		...
		//Options Menu
			   ...
			   g.drawImage(buttonStartGame, (int)towerSize*111/4, (int)towerSize*31/2, (int)towerSize*13/4, (int)towerSize*2, null);
				   ...
				   if(speed == 1){
					   g.drawImage(speedUpGame, (int)towerSize*111/4, (int)towerSize*31/2, (int)towerSize*13/4, (int)towerSize*2, null);
				   }else{
					   g.drawImage(speedUpGame2x, (int)towerSize*111/4, (int)towerSize*31/2, (int)towerSize*13/4, (int)towerSize*2, null);
				   }
			   }
		//Enemies
			for(int i = 0; i < enemyMap.length; i++){
		...
		//Health + Money + LevelNr
			...
			g.drawString("Money: " + user.player.money, (int)(12*towerSize/25) + 25, (15 * (int) towerSize) + (int)(12*towerSize/25) + 10 + (int) towerSize + frameHeightBorder);
			...
			g.drawString("Wave: " + wave.waveNumber, (int)(12*towerSize/25) + 25, (15 * (int) towerSize) + (int)(12*towerSize/25) + 40 + (int) towerSize + frameHeightBorder);
		...
		//Tower List
			for(int x = 0; x < 18; x++){
				for(int y = 0; y < 2; y++){
					if(Tower.towerList[x * 2 + y] != null){ 
						g.drawImage(Tower.towerList[x * 2 + y].texture, (int) ((int)(12*towerSize/25) + (int)(12*towerSize/25) + (int) (frameWidth / 11.52) + this.frameWidth / 40 + (int)(12*towerSize/25) + (x * towerSize)) , (15 * (int) towerSize) + (int)(12*towerSize/25) + (y * ((int)towerSize))+ frameHeightBorder, (int) towerSize, (int) towerSize, null);
						if(Tower.towerList[x * 2 + y].cost > user.player.money){
							g.setColor(new Color(255, 0, 0, 100));
							g.fillRect((int) ((int)(12*towerSize/25) + (int)(12*towerSize/25) + (int) (frameWidth / 11.52) + this.frameWidth / 40 + (int)(12*towerSize/25) + (x * towerSize)), (15 * (int)towerSize) + (int)(12*towerSize/25) + (y * ((int)towerSize)) + frameHeightBorder, (int) towerSize, (int) towerSize);
						}
					}
					g.setColor(Color.GRAY);
					g.drawRect((int) ((int)(12*towerSize/25) + (int)(12*towerSize/25) + (int) (frameWidth / 11.52) + this.frameWidth / 40 + (int)(12*towerSize/25) + (x * towerSize)), (15 * (int)towerSize) + (int)(12*towerSize/25) + (y * ((int)towerSize)) + frameHeightBorder, (int) towerSize, (int) towerSize);
				}
			...
			if(hand != 0 && Tower.towerList[hand - 1] != null){
				g.drawImage(Tower.towerList[hand - 1].texture, this.handXPos - (int) Screen.towerSize / 2 , this.handYPos - (int) this.towerSize / 2, (int) towerSize, (int) towerSize, null);
			}
		}else{
	...
	public void startGame(User user, String level){
		...
		System.out.println(this.enemyAI.route.getPoinstWorth(2, 4));
	...
	int frames = 0;
	public void run(){
		System.out.println("[Sucesso] Frame Criado!");
		
		long lastFrame = System.currentTimeMillis();
		int syncornized_fps = 0;
		...
		while(running){
			...
			if(timeMilliSec > syncornized_fps * 1000 / 25){ 
				syncornized_fps++;
				update();
				
				if(syncornized_fps == 1000 / 25){
					syncornized_fps = 0;
				}
			}
			
			if(timeMilliSec + 1000 / 25 < syncornized_fps * 1000 / 25){
				syncornized_fps = 0;
			}
	...
	public void enemyUpdate(){
		for(int i = 0; i < enemyMap.length; i++){
			if(enemyMap[i] != null){
				...
				int moneyEarnedIfDead = Enemy.enemyList[enemyMap[i].id].health;
				
				enemyMap[i] = enemyMap[i].update(); // Currently Only To check if they are dead or not
