...
public abstract class Tower implements Cloneable{
	...
	public static final Tower LightningTowerGreen = new TowerLightning(1, 25, 2, 3, 6, 15).getTextureFile("LightningTower2");
	public static final Tower LightningTowerBlue = new TowerLightning(2, 50, 2, 4, 6, 12).getTextureFile("LightningTower3");
	public static final Tower LightningTowerPink = new TowerLightning(3, 75, 2, 5, 9, 21).getTextureFile("LightningTower4");
	public static final Tower missileTower = new TowerMissile(4, 10, 3, 10, 0, 50).getTextureFile("MissileTower");
	...
	public int RANDOM = 1; /** Attack enemy nearest base */
	public int FIRST = 2; /** Attack random enemy */
	public int LAST = 3; /** Attack enemy nearest base */
			...
			for(int i = 0; i < enemiesInRange.length; i++){
				if(enemiesInRange[i] != null){
					if(bestTarget == null){
						...
						int b_points_worth = Screen.enemyAI.route.getPoinstWorth(b_x, b_y);
						
						if(Screen.enemyAI.route.getPoinstWorth(enemiesInRange[i].routePosX, enemiesInRange[i].routePosY) > b_points_worth){
							bestTarget = enemiesInRange[i];
						}else if(Screen.enemyAI.route.getPoinstWorth(enemiesInRange[i].routePosX, enemiesInRange[i].routePosY) == b_points_worth){
